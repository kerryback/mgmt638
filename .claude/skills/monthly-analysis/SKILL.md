...
# Monthly Stock Analysis Expert

You are an expert in monthly stock analysis, fetching data from the Rice Data Portal and providing complete analysis-ready datasets.

## OVERVIEW

This skill provides a **complete, self-contained workflow** for monthly stock analysis:
1. **Fetch** end-of-month prices and valuation metrics from Rice Data Portal
2. **Calculate** monthly returns and momentum
3. **Fetch** fundamental data from 10K/10Q filings
4. **Merge** all data into analysis-ready monthly dataset
...
- ✅ **In DAILY table?** → Fetch from DAILY (then shift by 1 month)
- ✅ **In SF1 table?** → Fetch from SF1 (then shift by 1 month)
...
**ALL variables from DAILY and SF1 MUST be shifted by 1 month** (grouped by ticker):
```python
...
This avoids look-ahead bias - variables represent prior month's values.

---

## UTILITY SCRIPTS

**Located in `.claude/skills/monthly-analysis/`:**

### 1. Fetch Monthly Returns (with Marketcap)

```bash
python .claude/skills/monthly-analysis/fetch_monthly_data.py START_DATE OUTPUT_FILE
# For all stocks: python .claude/skills/monthly-analysis/fetch_monthly_data.py 2020-01-01 monthly.parquet
# For specific tickers: python .claude/skills/monthly-analysis/fetch_monthly_data.py AAPL,MSFT 2020-01-01 monthly.parquet
```

**Automatically includes:**
- `ticker`, `month`, `date`
- `close`, `return`, `momentum`, `lag_month`
...
```bash
python .claude/skills/monthly-analysis/merge_monthly_fundamentals.py MONTHLY_FILE FUNDAMENTALS_FILE OUTPUT_FILE
```

**Automatically handles:**
- First month AFTER filing date
...
```bash
python .claude/skills/monthly-analysis/fetch_monthly_data.py 2010-01-01 monthly_returns.parquet
```

This gets: ticker, month, date, close, return, momentum, lag_month, marketcap (shifted), sector, industry, size

### Step 2: Fetch Additional DAILY Variables (if needed)

**Example: If you need pb from DAILY table:**

Write a Python script to fetch pb from DAILY table (year-by-year), then shift by 1 month.
...
```bash
python .claude/skills/monthly-analysis/merge_monthly_fundamentals.py monthly_returns.parquet fundamentals.parquet merged.parquet
```

### Step 5: Shift SF1 Variables

After merging, shift all SF1 variables by 1 month:
```python
df['roe'] = df.groupby('ticker')['roe'].shift(1)
...
### End-of-Month/Week Filtering

**For monthly end-of-month prices (SEP table):**
```sql
WITH month_ends AS (
...
### Monthly Returns and Momentum

**After fetching end-of-month prices:**
```python
...
- Expressed as DECIMALS (0.05 = 5% return)
- Rounded to 4 decimal places
- First month has NaN return
- First 13 months have NaN momentum

---

## IMPORTANT NOTES

1. **Month labels are strings**: '2025-01', '2025-02', etc.
...
6. **All DAILY/SF1 variables must be shifted**: By 1 month after fetching

For complete database documentation:
- https://portal-guide.rice-business.org
- https://data-portal.rice-business.org
