...
print(f"  Months: {df_clean['month'].nunique()}")
print(f"  Month range: {df_clean['month'].min()} to {df_clean['month'].max()}")

# Save as data5_monthly.parquet
df_clean.to_parquet('data5_monthly.parquet', index=False)
print(f"\nSaved cleaned data to data5_monthly.parquet")
