"""
Merge freshly fetched weekly returns with annual fundamentals.
Fundamentals become available the week after the filing date.
"""
import pandas as pd
import numpy as np

print("Loading weekly returns (fresh data with corrected calculations)...")
df_weekly = pd.read_parquet('weekly_raw.parquet')
...
# Rename lag_month to lagged_return to match data4.parquet naming
print("\nRenaming lag_month to lagged_return...")
df_merged = df_merged.rename(columns={'lag_month': 'lagged_return'})

# Drop date and lag_week columns (not in data4.parquet structure)
print("\nDropping date and lag_week columns...")
df_merged = df_merged.drop(columns=['date', 'lag_week'])

# Select final columns to match data4.parquet structure
...
# Save
df_final.to_parquet('data5_merged.parquet', index=False)
print(f"\nSaved {len(df_final)} rows to data5_merged.parquet")
